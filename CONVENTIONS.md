...
| **change journal records** | `usn`'s counts: `records` per `location`, and of those `created`, `deleted`, `renamed`, `data_changed`; on the journal's own observation also `records_version_2`/`_3`/`_4`, `trimmed`, `maximum_size`. Never a name (ADR 0047) | `rongroi_collectors::usn` |
