...
| **discriminator** | The observation field a collector that reads several places uses to say which one an observation is about — today only `fivem_dir`'s `location`. A place that could not be read is a gap for that place's observations only (ADR 0044) | `rongroi_collectors::Collector::discriminator`, `rongroi_core::model::DiscriminatorGaps` |
