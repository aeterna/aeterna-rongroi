...
| **discriminator** | The observation field a collector that reads several places uses to say which one an observation is about — today `fivem_dir` and `usn`'s `location`. A place that could not be read is a gap for that place's observations only, and a baseline observation that differs from a rule in the discriminator alone does not confront it (ADR 0044, ADR 0033) | `rongroi_collectors::Collector::discriminator`, `rongroi_core::model::DiscriminatorGaps` |
| **location** | The discriminator field of `fivem_dir` and `usn`: which place an observation is about. The two collectors use the same value for the same folder (`plugins`, `enhanced_asi`) | observation field `location` |
