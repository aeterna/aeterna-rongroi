...
| M3 | Vulnerable-driver list, USN journal, Amcache, screenshare guide | the [screenshare guide](docs/screenshare-guide.md) is released in 0.3.0. Amcache is decided against for now: no collector and no hash rules until the conditions in [ADR 0041](docs/adr/0041-amcache-feasibility.md) are met. The vulnerable-driver list is designed in [ADR 0046](docs/adr/0046-vulnerable-drivers-feasibility.md) and measured on a GitHub-hosted runner and a Windows 11 PC, with the LOLDrivers list counted; the `driver_service` collector lists registered driver services with each file's SHA-256 ([ADR 0048](docs/adr/0048-the-driver-service-collector.md)), and a `posture` rule compares them with 1,847 verified LOLDrivers hashes; on a Windows 11 PC with a hardware utility's drivers it reads found. The USN journal collector reads the Windows drive's change journal without write access and counts records per watched folder, with no file names ([ADR 0047](docs/adr/0047-usn-journal-feasibility.md)); no rule reads it yet |
| M4 | Which Windows a PC runs | released in 0.4.0: `os_image` reads what Windows says this installation is — the edition and build values `winver` shows, the registered organisation, the manufacturer, model and support link Settings shows, and how each of eight services Windows ships with is set to start ([ADR 0056](docs/adr/0056-what-windows-says-this-installation-is.md)); `install_marker` answers, for six named places, whether each is there — five the Atlas and ReviOS playbooks install and Defender's platform folder, read for its absence ([ADR 0057](docs/adr/0057-named-places-and-the-mitigation-switches.md)); and `posture` gained the speculative-execution, SEHOP and object-namespace switches a tweak script flips. Thirteen rules, all `posture`, all `experimental`, each saying that a person may run whatever operating system they like on their own PC. A pre-modified ISO that names itself nowhere — KernelOS, Ghost Spectre, a `tiny11builder` image — is identified only by what is missing |

The releases so far are 0.1.0, 0.2.0, 0.3.0 and 0.4.0. The release pipeline shipped with 0.1.0
