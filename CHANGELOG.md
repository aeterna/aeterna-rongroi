...
- Listed counts: every report view carries how many of the evidence it lists are found, not found and
  ...
  one (ADR 0045).
- The code link: the text report ends with where to read this binary's code — the commit of an
  official build, or the repository with a note that the code of an unofficial build is not known.
  Rule text now carries each rule's status and where its rule, fixtures and collector are in the
  repository, checked to exist by a test (ADR 0045).
