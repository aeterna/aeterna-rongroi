...
A scan is `standard` or `full`, chosen before it starts by the process that reads (ADR 0052). Each collector
declares its tier (`Collector::tier`, `standard` unless its ADR says otherwise); `scan::run` takes the tier in
`ScanContext` and does not call a collector above it, whose run is then `not_consented`. The report header
carries `scan_tier`. The CLI asks with `scan --full` on standard error, and only `yes` starts a full scan; the
desktop app asks in a Windows dialog (`rongroi_host_windows::dialog`) in a copy started with `--full`
(`rongroi_host_windows::relaunch`, the same token), before the scan and before any WebView exists. Today one
collector is `full`: `fivem_servers` (ADR 0055).
...
| `unmeasured` with reason `not_admin`, `not_attempted` or `not_consented` | shown, and in the scope statement | **not** a row — one fact about the scan, said once above the evidence in `scope.not_admin` / `scope.not_attempted` / `scope.not_consented` (ADR 0012, ADR 0027, ADR 0030, ADR 0052) |
| A field a `full` collector declares sensitive | as read | replaced by `%SERVER_IDENTITY%` or `%ACCOUNT_IDENTIFIER%` everywhere in the view — rows, timeline, own traces, unmatched — unless the player agreed to show that kind; `view::for_mode_with` takes the answers as `SsOptions` (ADR 0052) |
...
| `fivem_servers` | **Full scan only** (ADR 0052). `%LOCALAPPDATA%\FiveM for GTAV Enhanced\servercache`, listed once and nothing below it: `folder` (`listed`, `absent`, `unreadable`) and `server_folders`; and for each folder named with 40 hexadecimal characters, its name as `server_folder` — declared sensitive, a server identity that SS mode replaces with `%SERVER_IDENTITY%` unless the player agreed to show server identities — and its `created_at` and `modified_at` from the listing. What the name is computed from is not established. A folder of any other name is not reported, and nothing inside a server folder is listed or opened (ADR 0055) | not recorded: the folder is under the player's own `%LOCALAPPDATA%`, and a listing with a limited token was not measured (ADR 0055) | M4 · ADR 0052, ADR 0055 |
...
| `os_image` | What Windows says this installation is, and how the services Windows ships with are set to start (ADR 0056). One observation, in the shape `posture` uses: a value that could be read is a field, one that could not is a `gaps` entry under that field's own name. **Identity**, from `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion`: `product_name`, `edition_id`, `composition_edition_id`, `display_version`, `current_build`, `ubr`, `build_lab_ex`, `installation_type`, `registered_organization`. `RegisteredOwner` is **not** read — it holds a person's name. **OEM information**, from `HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\OEMInformation`: `oem_manufacturer`, `oem_model`, `oem_support_url`, the values Settings shows and the ones the Atlas and ReviOS playbooks write their own name into. **Services**, from `HKLM\SYSTEM\CurrentControlSet\Services`: one text field for each of `WinDefend`, `wuauserv`, `WerSvc`, `EventLog`, `SysMain`, `DPS`, `WSearch` and `DiagTrack` — `boot`, `system`, `automatic`, `manual`, `disabled`, or `absent` when the service's key is not there, which is a value and not a gap. Read only once the key they live under has been listed; when it has not, every service field is a gap instead | no, measured 2026-09-20 on one Windows 11 PC (build 26220) with an elevated token only, so `access_denied` stays declarable | ADR 0056 |
| `install_marker` | Whether each of six named places is on this PC (ADR 0057), one observation for **every** one of them, `present: true` or `present: false`, told apart by `marker`, its discriminator: the canonical spelling with the environment variable unexpanded (`%SystemRoot%\AtlasModules`), which is what a rule matches; `path` carries where this PC puts it and `kind` is `folder` or `registry_key`. Five are what the Atlas and ReviOS playbooks install — `%SystemRoot%\AtlasModules`, `%SystemRoot%\AtlasDesktop`, `HKLM\SOFTWARE\AtlasOS`, `%ProgramFiles%\Revision Tool`, `%SystemRoot%\Web\Wallpaper\MeetRevision` — and the sixth is Microsoft's own `%ProgramData%\Microsoft\Windows Defender\Platform`, read for its absence beside `os_image`'s `service_windefend`. A folder is listed for one bit and **its entries are dropped**; a registry key's values are counted for nothing but existence. A place whose environment variable is not set, or whose read was refused, is a `DiscriminatorGaps` for that place alone | no, measured 2026-09-21 on one Windows 11 PC (build 26220) with a limited token (ADR 0057) | ADR 0057 |
