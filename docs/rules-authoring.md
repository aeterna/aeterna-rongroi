...
| `allow` | no | legitimate software excluded by `sha256` or `signer` — never by file name. `prefetch`, `bam` and `pca` emit neither, so a rule on them has nothing to allow by, and no rule on them names a program by `name` or `path` (ADR 0034). No gate refuses that rule; review does |
